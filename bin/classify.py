...
#gSystem.Load('rhoCorrectionWeight_C.so')
...
#            chain.AddFriend('TF_'+str(name.split('/')[-1])+'_'+str(ttype),'procID_'+str(name.split('/')[-1])+'_'+str(ttype)+'.root')
    ...
    defaultSettings = { "BDT" :  "!H:!V:!CreateMVAPdfs:BoostType=Grad:UseBaggedBoost:NegWeightTreatment=NoNegWeightsInTraining"
    ...
    if "Cuts" in o.methods:
        ...
        if len(categories) == 0:
            ...
            for cut,name,vars in categories:
                print "booking sub-category classifier : %s %s %s" % ( cut, name, vars )
                cats.AddMethod(cut,vars,TMVA.Types.kCuts,"%s_%s" % (mname,name),settings)
